...
/**
 * GolfTheme — Material 3 theme for Golf Performance Tracker.
 *
 * Dynamic color (Material You) is OFF — this is a branded experience.
 * [accent] re-skins only primary/secondary/tertiary roles; surface tones stay neutral.
 *
 * Persist ThemeMode (System/Light/Dark) and GolfAccent in DataStore, then pass here.
 */

/** User-selectable accent palette. */
enum class GolfAccent { Fairway, Ocean, Amber, Graphite }

// ── Base schemes (Fairway seed) ─────────────────────────────────────────
...
// ── Accent override — primary / secondary / tertiary roles only ─────────
...
fun GolfTheme(
    ...
    val baseScheme = if (darkTheme) FairwayDark else FairwayLight
    MaterialTheme(
        colorScheme = applyAccent(baseScheme, darkTheme, accent),
